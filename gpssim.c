...
int src_nst(double dis)
{
	...
	while(l <= r)
	{
		...
		if (d[c][3] < dis && d[c+1][3] > dis) return c;
		if (d[c-1][3] < dis && d[c][3] > dis) return c - 1;
		else if (d[c][3] > dis) r = c - 1;
		else if (d[c][3] < dis) l = c + 1;
		else return c;
	}
}

void w_ave(int n, double dis, double *lled)
{
	lled[0] = (fabs(dis - d[n+1][3]) * d[n][0] + fabs(dis - d[n][3]) * d[n+1][0]) / (d[n+1][3] - d[n][3]);
	lled[1] = (fabs(dis - d[n+1][3]) * d[n][1] + fabs(dis - d[n][3]) * d[n+1][1]) / (d[n+1][3] - d[n][3]);
	lled[2] = (fabs(dis - d[n+1][3]) * d[n][2] + fabs(dis - d[n][3]) * d[n+1][2]) / (d[n+1][3] - d[n][3]);
 ...
 */
int readUserMotion(double xyz[USER_MOTION_SIZE][3], const char *filename, double v)
{
	FILE *fp;
	int numd;
	char str[MAX_CHAR];
	...
	for (numd=0; numd<USER_MOTION_SIZE; numd++)
	{
		...
		if (EOF==sscanf(str, "%lf,%lf,%lf,%lf", &d[numd][0], &d[numd][1], &d[numd][2], &d[numd][3])) // Read CSV line
			break;
		
			d[numd][0] = d[numd][0] / R2D;
			d[numd][1] = d[numd][1] / R2D;
		//printf("d[%d][3]=%lf\n",numd,d[numd][3]);
	...
	for (i=0; i < n_sam; i++)
	{
		...
		llh2xyz(lled,rocal_xyz);
		//printf("xyz= %lf  %lf  %lf\n", rocal_xyz[0],rocal_xyz[1],rocal_xyz[2]);
		xyz[i][0] = rocal_xyz[0];
		xyz[i][1] = rocal_xyz[1];
		xyz[i][2] = rocal_xyz[2];
		//printf("xyz[%d] = %jf, %jf, %jf\n",i,xyz[i][0],xyz[i][1],xyz[i][2]);
	}
...
int main(int argc, char *argv[])
{
	...
	while ((result=getopt(argc,argv,"e:u:V:g:c:l:o:s:b:T:t:d:iv"))!=-1)
	{
		switch (result)
		{
		case 'e':
	...
	if (!staticLocationMode)
	{
		...
		if (nmeaGGA==TRUE)
			...
			numd = readUserMotion(xyz, umfile, v);
